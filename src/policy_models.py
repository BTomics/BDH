...
class BDHActorCritic(nn.Module):
    """
    BDH policy wrapped for PPO. Outputs a normal distribution for actions and a value estimate.
    """
    def __init__(self, config: bdh.BDHConfig):
        super().__init__()
        self.config = config
    ...
    def forward(self, states):
        ...
        features = x.view(B, T, D)
        
        # Scale mean to [-2.0, 2.0] for Pendulum
        action_mean = 2.0 * torch.tanh(self.actor_mean(features))
