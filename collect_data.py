...
if __name__ == "__main__":
    ...
    np.savez_compressed("train_data.npz", states=train_data["states"], actions=train_data["actions"])
    np.savez_compressed("val_data.npz", states=val_data["states"], actions=val_data["actions"])
    
    print("Datasets saved successfully.")
    
    # 4. Sanity check: load the saved files and print their shapes
    loaded_train = np.load("train_data.npz")
    loaded_val = np.load("val_data.npz")
